...
export const metadata: Metadata = {
  title: "Usługi — każda z ceną, zakresem i terminem",
  description:
...
const SERVICES = [
  {
    ...
    sla: "oferta w 48 h",
    scope:
      "Wyszukanie dostawcy, negocjacje, kontrakt, kontrola produkcji, inspekcja, transport, odprawa i dostawa pod drzwi. Jedna umowa, jedna faktura VAT.",
    badge: "bestseller",
  },
  {
    name: "Import z 1688.com",
    price: "prowizja od 5%",
    sla: "wycena koszyka w 24 h",
    scope:
  ...
  {
    name: "Weryfikacja dostawcy w 48 h",
    price: "499 zł",
    sla: "raport w 48 h",
    scope:
      "Dokumenty rejestrowe AIC, licencja eksportowa, kapitał, sprawy sądowe, referencje, rozmowa telefoniczna po chińsku. Ocena punktowa + rekomendacja.",
  },
  {
  ...
  },
  {
    ...
    price: "od 1 180 zł",
    sla: "raport w 24 h od wizyty",
    scope:
      "Kontrola partii wg AQL 2,5 przed wysyłką: wymiary, funkcje, opakowania, oznakowanie CE. Decyzja ZALICZONA/ODRZUCONA zanim zapłacisz dopłatę.",
  },
  {
  ...
  },
  {
    ...
    price: "od 52 USD/m³",
    sla: "rezerwacja w 24 h",
    scope:
      "Morze LCL/FCL, kolej, lotniczy. Odprawa we własnej agencji celnej, procedura 4200, składy celne. Śledzenie kontenera w panelu.",
  },
  {
      ...
      <div className="mt-10 grid gap-px overflow-hidden rounded-xl border border-line bg-line md:grid-cols-2">
        {SERVICES.map((s) => (
          <div key={s.name} className="relative bg-card p-6">
            {s.badge && (
              <span className="absolute right-6 top-6 rounded-md border border-accent/50 px-2 py-0.5 font-mono text-[10px] font-semibold uppercase tracking-[0.08em] text-accent">
                {s.badge}
              </span>
            )}
            ...
            <button className="btn-anim mt-5 rounded-lg bg-ink px-4 py-2 text-[13px] font-medium text-paper hover:opacity-85">
              Zamów ↗
            </button>
          </div>
        ))}
